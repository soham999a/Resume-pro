...
def analyze_resume_with_openai(resume_text):
    try:
        ...
        with open(log_file, 'a') as f:
            f.write('-' * 80 + '\n')
            f.write('Analyzing resume with OpenAI API...\n')
            f.write(f'Resume text length: {len(resume_text)}\n')
            f.write(f'Resume text sample: {resume_text[:100]}...\n')
            f.write(f'API Key: {"*" * 5}{OPENAI_API_KEY[-4:] if OPENAI_API_KEY else "Not set"}\n')
            f.write('-' * 80 + '\n')
        ...
        try:
            ...
            # Log to file
            with open(log_file, 'a') as f:
                f.write('OpenAI API response received successfully\n')
...
def run_server(port=3003):
    ...
    with open(log_file, 'w') as f:
        ...
        f.write(f'OpenAI API Key: {"*" * 5}{OPENAI_API_KEY[-4:] if OPENAI_API_KEY and OPENAI_API_KEY != "your-openai-api-key-here" else "Not set"}\n')
